...
「アプリで開く」は、公開中の [modellogue.com/app](https://modellogue.com/app) でそのまま開く例です（アプリはサイト直下ではなく **`/app`** 配下にあります）。自分で別のアドレスに
公開している場合は、`https://modellogue.com/app` の部分をそのアドレスに読み替えてください。手元に保存した `.json` は、
